...
export async function getAllEpisodes(seriesId?: string): Promise<Episode[]> {
  ...
  try {
    ...
    if (serverEpisodes && Array.isArray(serverEpisodes)) {
      // Merge server episodes with local episodes so locally saved episodes are never lost
      const map = new Map<string, Episode>();
      serverEpisodes.forEach((ep) => map.set(ep.id, ep));
      localList.forEach((ep) => {
        if (!seriesId || ep.seriesId === seriesId) {
          if (!map.has(ep.id)) {
            map.set(ep.id, ep);
          }
        }
...
export async function saveEpisode(episode: Episode): Promise<void> {
  ...
  // 3. Try syncing metadata to the server
  try {
    await saveEpisodeToServer(episode);
  } catch (err) {
    console.warn('Aviso: Servidor retornou erro ao sincronizar episódio. Episódio salvo com sucesso no dispositivo:', err);
    // Do NOT throw: the episode is safely stored and fully playable!
  }
}
